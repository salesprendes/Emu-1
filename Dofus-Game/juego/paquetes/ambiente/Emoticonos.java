	...
	public void analizar(JuegoSocket socket, String paquete)
	{
		...
		switch (emote) 
		{
			...
			case 21: //Campeon
				...
				{
					personaje_cuenta.get_Localizacion().get_Mapa().get_Personajes().stream().filter(filtro -> filtro.get_Esta_Conectado() && filtro.get_Id() != personaje_cuenta.get_Id()).forEach(personaje -> 
					{
		...
		personaje_cuenta.get_Localizacion().get_Mapa().get_Personajes().stream().filter(personaje -> personaje.get_Esta_Conectado()).forEach(personaje -> personaje.get_Cuenta().get_Juego_socket().enviar_Paquete("eUK" + personaje_cuenta.get_Id() + '|' + personaje_cuenta.get_Emote_Activo()));
	}
}
