	...
	public void analizar(JuegoSocket socket, String paquete)
	{
		if (socket.get_Cuenta().get_Rango_cuenta() > 0) 
		{
			try 
			{
				...
				final Mapas mapa = Mapas.get_Mapa_Coordenadas_SubArea(X, Y, socket.get_Personaje().get_Mapa().get_Sub_area().get_Area().get_Super_area().get_Id());
		
				socket.get_Personaje().get_Teleport(mapa.get_Id(), (short) 250);
