...
public class Smileys implements GestorPaquetes
{
	public void analizar(JuegoSocket socket, String paquete)
	{
		try 
		{
			...
			socket.get_Personaje().get_Mapa().get_Personajes().forEach(personaje -> personaje.get_Cuenta().get_Juego_socket().enviar_Paquete("cS" + socket.get_Personaje().get_Id() + '|' + smiley_id));
		} 
